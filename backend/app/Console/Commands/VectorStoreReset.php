...
class VectorStoreReset extends Command
{
    ...
    protected $description = 'ベクトルストア内の全ファイルを削除し、DBの同期ステータスをリセットします';

    public function handle(VectorStoreService $service): int
    {
        if (!$this->option('force') && !$this->confirm('ベクトルストアの内容をすべて削除し、DBのステータスをリセットしますか？')) {
        ...
        try {
            ...
            $this->info("   ✓ ベクトルストアからの解除: {$stats['vector_store']} 件");
